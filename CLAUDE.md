...
The first slice (core round loop with restart) is implemented, playtested, and merged, the pixel-art visual pass is implemented, the upgrade tree is implemented, and enemy progression (new enemy types, round-over-round escalation, a boss every 5th round) is implemented. Design specs: `docs/superpowers/specs/2026-08-18-round-loop-design.md`, `docs/superpowers/specs/2026-08-18-pixel-art-feel-design.md`, `docs/superpowers/specs/2026-08-18-upgrade-tree-design.md`, and `docs/superpowers/specs/2026-08-19-enemy-progression-design.md` (executed plans alongside them in `docs/superpowers/plans/`).
...
- `game_state.gd` — autoload `GameState`: persistent `currency` plus the `stats` dictionary (fire_interval, damage, target_size, round_duration, spawn_interval, initial_enemies, currency_per_kill), always recomputed from `BASE_STATS` + `upgrade_levels` by `buy_upgrade()`/`_recompute_stats()` (idempotent, no drift). **This is the upgrade seam**: all gameplay reads tunables from `GameState.stats` at point of use; the upgrade tree mutates `upgrade_levels` between `_on_round_over` and `start_round` — never hard-code a value that has a stats key. Also tracks round/escalation state, session-only like `currency`: `round_number`, `toughness_level`, and `frenzy`, advanced by `advance_round()` (called at the start of every round, from round 6 on it either bumps `toughness_level` or rolls a one-round `frenzy`) and read via `hp_mult()`/`speed_mult()`.
...
- `main.gd` / `main.tscn` — owns the round lifecycle: spawning into the `Enemies`/`Pickups`/`Effects` container nodes (`start_round()` clears them defensively before every round), picking each spawn uniformly from `EnemyTypes.unlocked(round_number)`, capping live enemies at `MAX_ENEMIES` (20) and adapting the spawn interval (0.5 s once the alive count drops below 6, else `stats.spawn_interval`), routing `fired(rect)` to enemies, countdown + currency HUD (CanvasLayer), and a brief "ROUND N" splash (hold then fade) at the start of every round. Round over hides the HUD and the reticle and mounts `upgrade_tree.tscn` onto `Screens`; pressing Start Round frees the tree and resumes. Cursor rules: the OS cursor is hidden during rounds (`Input.MOUSE_MODE_HIDDEN` — the reticle is the pointer) and shown as the pixel-art cursor (`sprites/cursor.png`, hotspot `(0,0)`) on the upgrade screen. Node order matters: `TargetingArea` is deliberately last among Node2Ds so it draws on top.
- `enemy.gd` (`class_name Enemy`) — wanders (random velocity, re-rolled every 1–3 s), bounces off viewport edges, uses `global_position` consistently; sized, HP'd, and paid out via `setup(def, hp_mult, speed_mult)` (an `EnemyTypes` def plus the round's scaling multipliers), which must be called before `add_child` since `_ready()` reads `max_hp`/`_def` to build the sprite (falls back to an unscaled rabbit if `setup` was skipped). Damage is shown by `shaders/enemy_damage.gdshader` (`hp_ratio` uniform, set in `take_damage`): the purple body erodes to peach left → right; fully peach = dead. Emits `died(at_position, coins)` before `queue_free()`.
...
- Sprite assets (`sprites/`): PNG with transparent background, authored at 1:1 base-resolution scale (no pre-upscaling), exactly the four palette colors, enemy 24×24 (rabbit, pig) / giant 32×32 (giant_rabbit, giant_pig) / boss_rabbit 48×48 / drop 4×4 / upgrade icons 16×16 (`icon_dmg`, `icon_size`, `icon_speed`, `icon_hub`), same file names to hot-swap. Enemy art must keep `Palette.PURPLE` as the body color so the damage shader can find it. Exception: `sprites/cursor.png` is pre-upscaled ×4 to 32×32, because OS cursors render at window resolution, not the 480×270 canvas. Placeholders are regenerated by `"$GODOT" --headless --path . --script res://tools/make_placeholders.gd`.

## Next slice / known deferred work

- Plausible next slices: save/load persistence for `currency` and `upgrade_levels` across sessions, further balancing round difficulty against upgraded stats so later rounds stay challenging (the round-6+ escalation split — 30% frenzy chance, 20%/level toughness HP step, 1.5× frenzy speed, all in `game_state.gd` — are first-guess numbers pending playtest), deeper upgrade-tree tiers beyond the current 3-level pips.
